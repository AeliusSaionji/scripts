...
$webclient.DownloadFile( "https://github.com/msysgit/msysgit/releases/download/Git-1.9.2-preview20140411/Git-1.9.2-preview20140411.exe", "$Env:TEMP\Git-1.9.2-preview20140411.exe" )
...
Write-Host "Creating curl.cmd for vim"
Write-Warning "Do not continue until git has been installed"
pause

$stream = [System.IO.StreamWriter] "C:\Program Files (x86)\Git\cmd\curl.cmd"
$stream.WriteLine("@rem Do not use `"echo off`" to not affect any child calls.")
$stream.WriteLine("@setlocal")
$stream.WriteLine("")
$stream.WriteLine("@rem Get the abolute path to the parent directory, which is assumed to be the")
$stream.WriteLine("@rem Git installation root.")
$stream.WriteLine("@for /F `"delims=`" %%I in (`"%~dp0..`") do @set git_install_root=%%~fI")
$stream.WriteLine("@set PATH=%git_install_root%\bin;%git_install_root%\mingw\bin;%PATH%")
$stream.WriteLine("")
$stream.WriteLine("@if not exist `"%HOME%`" @set HOME=%HOMEDRIVE%%HOMEPATH%")
$stream.WriteLine("@if not exist `"%HOME%`" @set HOME=%USERPROFILE%")
$stream.WriteLine("")
$stream.WriteLine("@curl.exe %*")
$stream.close()
